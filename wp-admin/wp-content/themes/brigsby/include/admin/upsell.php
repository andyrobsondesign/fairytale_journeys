...
function hoot_appearance_subpage() {
	...
	add_theme_page(
		__( 'Brigsby Premium', 'brigsby' ),	// Page Title
		__( 'Premium Options', 'brigsby' ),	// Menu Title
		'edit_theme_options',						// capability
		'brigsby-premium',							// menu-slug
...
function hoot_admin_enqueue_upsell_styles( $hook ) {
	if ( $hook == 'appearance_page_brigsby-premium' )
...
function hoot_theme_appearance_subpage() {
	...
	$hoot_cta = ( empty( $hoot_cta ) ) ? '<a class="button button-primary button-buy-premium" href="http://wphoot.com/" target="_blank">' . __( 'Click here', 'brigsby' ) . '</a>' : $hoot_cta;
	...
	$hoot_intro = wp_parse_args( $hoot_intro, array(
		'name' => __('Upgrade to Premium', 'brigsby'),
		'desc' => '',
		) );
	?>
	<div id="hoot-upsell" class="wrap">
		...
		<?php if ( !empty( $hybridextend_options_premium ) && is_array( $hybridextend_options_premium ) ): ?>
			<div class="hoot-upsell-sub">
				<?php foreach ( $hybridextend_options_premium as $key => $feature ) : ?>
 ...
 * @return void
 */
function hoot_theme_customizer_load_premiumfly(){
	return false;
}
add_filter( 'hybridextend_customize_load_premiumfly', 'hoot_theme_customizer_load_premiumfly' );
