	...
	<aside <?php hybridextend_attr( 'sidebar', 'primary' ); ?>>
		...
		if ( is_active_sidebar( 'hoot-primary-sidebar' ) ) : // If the sidebar has widgets.
			...
			the_widget(
				'WP_Widget_Text',
				array(
					'title'  => __( 'Example Widget', 'brigsby' ),
					/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
					'text'   => sprintf( __( 'This is an example widget to show how the Primary sidebar looks by default. You can add custom widgets from the %swidgets screen%s in wp-admin.', 'brigsby' ), current_user_can( 'edit_theme_options' ) ? '<a href="' . admin_url( 'widgets.php' ) . '">' : '', current_user_can( 'edit_theme_options' ) ? '</a>' : '' ),
					'filter' => true,
				),
				array(
			...
			if ( is_active_sidebar( 'hoot-secondary-sidebar' ) ) : // If the sidebar has widgets.
				...
				the_widget(
					...
					array(
						'title'  => __( 'Example Widget', 'brigsby' ),
						/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
						'text'   => sprintf( __( 'This is an example widget to show how the Secondary sidebar looks by default. You can add custom widgets from the %swidgets screen%s in wp-admin.', 'brigsby' ), current_user_can( 'edit_theme_options' ) ? '<a href="' . admin_url( 'widgets.php' ) . '">' : '', current_user_can( 'edit_theme_options' ) ? '</a>' : '' ),
						'filter' => true,
					),
					array(
						'before_widget' => '<section class="widget widget_text">',
