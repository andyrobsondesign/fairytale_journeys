	...
	<aside <?php hybridextend_attr( 'sidebar', 'primary' ); ?>>
		...
		if ( is_active_sidebar( 'hoot-woocommerce-sidebar' ) ) : // If the sidebar has widgets.
			...
			the_widget(
				'WP_Widget_Text',
				array(
					'title'  => __( 'Woocommerce Sidebar', 'brigsby' ),
					/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
					'text'   => sprintf( __( 'Woocommerce pages have a separate sidebar than the rest of your site. You can add custom widgets from the %swidgets screen%s in the admin.', 'brigsby' ), current_user_can( 'edit_theme_options' ) ? '<a href="' . admin_url( 'widgets.php' ) . '">' : '', current_user_can( 'edit_theme_options' ) ? '</a>' : '' ),
					'filter' => true,
				),
				array(
